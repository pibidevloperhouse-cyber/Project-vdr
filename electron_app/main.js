...
// 🔥 AUTO-LAUNCH: Detects if the app was opened by double-clicking a .vdr file
...
ipcMain.handle('verify-access', async (event, payload) => {
    try {
        ...
        // 🔥 STRICT DB QUERY: Fetches active template and logs the result
        let watermarkConfig = null;
        try {
            if (doc.company_id) {
                const { data: wm, error: wmErr } = await supabase.from('watermark_templates')
                    .select('*')
                ...
                if (wmErr) {
                    console.error("🚨 Supabase Query Error:", wmErr.message);
                }

                if (wm) {
                    console.log("✅ Successfully fetched template:", wm.name);
                    watermarkConfig = wm;
                    const filename = wm.logo_path || wm.logo_url;
                    if (filename) {
                        const { data } = supabase.storage.from('vdr-logos').getPublicUrl(filename.trim());
                        watermarkConfig.final_logo_link = data.publicUrl;
                    }
                } else {
                    console.log("⚠️ No active template found where present = true");
                }
            }
        } catch (wmErr) { console.log("Watermark engine error:", wmErr.message); }
...
//         //     watermark: watermarkConfig
//         // };
//         // 🔥 STRICT DB QUERY: Specific Company AND 'present' is TRUE
//         // 🔥 STRICT DB QUERY: Specific Company AND 'present' is TRUE
//         let watermarkConfig = null;
//         try {
//             if (doc.company_id) {
//                 // Fetch the single active template for the document's company
//                 const { data: wm, error: wmErr } = await supabase.from('watermark_templates')
//                     .select('*')
//                     .eq('company_id', doc.company_id)
//                     .eq('present', true)
...
//                     // 🔥 THE FIX: Officially ask the Bucket for the real image URL!
//                     // (Checking both logo_path and logo_url just in case your column name varies)
//                     const filename = wm.logo_path || wm.logo_url;

//                     if (filename) {
//                         const { data } = supabase.storage.from('vdr-logos').getPublicUrl(filename.trim());
//                         watermarkConfig.final_logo_link = data.publicUrl; // Store the official bucket link
//                     }
//                 }
//             }
//         } catch (wmErr) {
//             console.log("No active watermark template found for this company.");
//         }
